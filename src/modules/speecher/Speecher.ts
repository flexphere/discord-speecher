...
export class Speecher extends Base {
  ...
  async getVoiceConfig(id: string): Promise<VoiceConfig | undefined> {
    return (await db.query("select * from voices where user_id = ?", [
      id,
    ])) as VoiceConfig;
  }
  ...
  async createVoiceConfig(id: string): Promise<VoiceConfig> {
    ...
    await db.exec(
      "insert into voices (user_id, type, rate, pitch, active, filter) values (?, ?, ?, ?, ?, ?);",
      [id, voice.type, voice.rate, voice.pitch, voice.active, voice.filter]
    );
  ...
  async UpdateVoicePitch(pitch, member_id) {
    logger.debug(
      `updating voice config: pitch=${pitch} member_id=${member_id}`
    );
    await db.exec("update voices set pitch = ? where user_id = ?;", [
      pitch,
      member_id,
    ]);
  }
  ...
  async UpdateVoiceSpeed(speed, member_id) {
    logger.debug(
      `updating voice config: speed=${speed} member_id=${member_id}`
    );
    await db.exec("update voices set rate = ? where user_id = ?;", [
      speed,
      member_id,
    ]);
  }
  ...
  async UpdateVoiceType(voice_type, member_id) {
    logger.debug(
      `updating voice config: type=${voice_type} member_id=${member_id}`
    );
    await db.exec("update voices set type = ? where user_id = ?;", [
      voice_type,
      member_id,
    ]);
  }
  ...
  async UpdateVoiceFilter(filter, member_id) {
    logger.debug(
      `updating voice config: type=${filter.name} member_id=${member_id}`
    );
    await db.exec("update voices set filter = ? where user_id = ?;", [
      filter.name,
      member_id,
    ]);
  }
  ...
  async ActivateVoiceStatus(member_id) {
    ...
    await db.exec("update voices set active = 1 where user_id = ?;", [
      member_id,
    ]);
  }
  ...
  async DeactivateVoiceStatus(member_id) {
    ...
    await db.exec("update voices set active = 0 where user_id = ?;", [
      member_id,
    ]);
  }
}
